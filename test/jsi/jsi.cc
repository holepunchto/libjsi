...
std::string
kindToString(const Value &v, Runtime *rt = nullptr) {
  if (v.isUndefined()) {
    return "undefined";
  } else if (v.isNull()) {
    return "null";
...
Value
callGlobalFunction(Runtime &runtime, const char *name, const Value &arg) {
...
void
encodeUTF16(std::u16string &out, uint32_t cp) {
  if (cp < 0x10000) {
...
// ASCII characters
bool
isAllASCII(const char16_t *utf16, size_t length) {
...
std::string
getUtf16CodeUnitString(const char16_t *utf16, size_t length) {
...
Value
HostObject::get(Runtime &, const PropNameID &) {
  return Value();
}

void
HostObject::set(Runtime &rt, const PropNameID &name, const Value &) {
...
Value
Runtime::createValueFromJsonUtf8(const uint8_t *json, size_t length) {
...
String
Runtime::createStringFromUtf16(const char16_t *utf16, size_t length) {
...
std::u16string
Runtime::utf16(const PropNameID &sym) {
  auto utf8Str = utf8(sym);
  return convertUTF8ToUTF16(utf8Str);
}

std::u16string
Runtime::utf16(const String &str) {
...
void
Runtime::setPrototypeOf(const Object &object, const Value &prototype) {
...
Value
Runtime::getPrototypeOf(const Object &object) {
  auto setPrototypeOfFn = global()
                            .getPropertyAsObject(*this, "Object")
...
Object
Runtime::createObjectWithPrototype(const Value &prototype) {
...
Function
Object::getPropertyAsFunction(Runtime &runtime, const char *name)
  const {
...
Array
Object::asArray(Runtime &runtime) const & {
  if (!isArray(runtime)) {
    throw JSError(
      runtime,
...
Array
Object::asArray(Runtime &runtime) && {
  if (!isArray(runtime)) {
    throw JSError(
      runtime,
      "Object is " + kindToString(Value(runtime, *this), &runtime) +
...
Function
Object::asFunction(Runtime &runtime) const & {
...
Function
Object::asFunction(Runtime &runtime) && {
  if (!isFunction(runtime)) {
    throw JSError(
      runtime,
      "Object is " + kindToString(Value(runtime, *this), &runtime) +
...
bool
Value::strictEquals(Runtime &runtime, const Value &a, const Value &b) {
  if (a.kind_ != b.kind_) {
    return false;
  }
  switch (a.kind_) {
...
Object
Value::asObject(Runtime &rt) const & {
  if (!isObject()) {
    throw JSError(
...
Symbol
Value::asSymbol(Runtime &rt) const & {
  if (!isSymbol()) {
    throw JSError(
...
BigInt
Value::asBigInt(Runtime &rt) const & {
  if (!isBigInt()) {
    throw JSError(
...
String
Value::asString(Runtime &rt) const & {
  if (!isString()) {
    throw JSError(
...
int64_t
BigInt::asInt64(Runtime &runtime) const {
  if (!isInt64(runtime)) {
...
void
JSError::setValue(Runtime &rt, Value &&value) {
  value_ = std::make_shared<Value>(std::move(value));
